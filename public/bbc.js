...
async function mountCatalogueMedia(entry, image, drive) {
  try { if (!confirmDriveDiscard(drive)) return; if (machine.bus.devices.fdc.transfer?.drive === drive) throw new Error(`Physical drive ${drive} has an active transfer; try again when it completes.`); const bytes = await fetchRom(`MEDIA/${image.filename}`); const wasRunning = running; stop(); if (machine.bus.devices.fdc.transfer?.drive === drive) { restoreRunStateAfterMediaMount(wasRunning); throw new Error(`Physical drive ${drive} has an active transfer; try again when it completes.`); } const disk = mountDriveSource(drive, bytes, { filename: image.filename, writeProtected: true, catalogueFilename: image.filename }); (drive ? elements.drive1WriteProtect : elements.drive0WriteProtect).checked = true; markCustomMediaSelection(); elements.persistenceDriveSelect.value = String(drive); elements.mediaState.textContent = disk.format.toUpperCase(); elements.mediaState.className = "status live"; const driveName = tubeEnabled ? `physical drive ${drive} / CP/M ${String.fromCharCode(65 + drive)}:` : `physical drive ${drive}; Acorn Z80 hardware is not active`; elements.mediaStatus.textContent = `${image.filename} mounted read-only in ${driveName}. Writes can only affect a private browser clone after write protection is disabled.`; elements.catalogueStatus.textContent = tubeEnabled ? `${entry.title}: ${image.filename} is mounted in drive ${drive}; status remains “${entry.status}”.` : `${image.filename} is mounted, but cannot run on the current BBC-only profile. Boot CP/M Utilities, then mount it in drive 1.`; const resumed = restoreRunStateAfterMediaMount(wasRunning); status(resumed ? "Catalogue media mounted; machine resumed. Type into the focused display." : "Catalogue media mounted; machine remains paused.", resumed ? "RUNNING" : "PAUSED"); draw(); }
