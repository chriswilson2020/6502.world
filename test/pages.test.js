...
test("Atom console exposes firmware, matrix input, debugger and portable states", async () => {
  ...
  for (const id of ["atomScreen", "atomTextMirror", "bootAtomButton", "resetAtomButton", "atomBasicInput", "atomFloatInput", "atomKernelInput", "atomPc", "atomCycles", "pauseAtomButton", "enableAtomAudioButton", "atomBreakpointInput", "stepAtomButton", "exportAtomStateButton", "atomStateInput"]) assert.match(html, new RegExp(`id=["']${id}["']`));
  assert.match(html, /2\.1 · INTERACTIVE ATOM/);
