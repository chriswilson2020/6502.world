...
test("BBC hardware profiles filter compatible software presets", () => {
  ...
  assert.deepEqual(softwareForProfile("bbc-model-b-acorn-z80").map(({ id }) => id), ["acorn-cpm-utilities", "acorn-cpm-bbc-basic", "acorn-cpm-memoplan", "custom-acorn-cpm"]);
  assert.equal(defaultSoftwareForProfile("bbc-model-b-acorn-z80"), "acorn-cpm-utilities");
  for (const preset of ["acorn-cpm-utilities", "acorn-cpm-bbc-basic", "acorn-cpm-memoplan"]) {
