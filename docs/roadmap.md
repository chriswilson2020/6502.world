...
**Implemented:** every approved DSD is represented exactly once with a verified SHA-256, placement, write-mode guidance, rights mode and evidence-based status. Utilities, BBC BASIC, MemoPlan and GraphPlan are validated launch entries. The BBC BASIC gate boots the known Utilities system in A:, selects the prepared Basic disc in B: through real keyboard input and reaches the 2.20 title prompt. The MemoPlan gate launches the 1.30 editor and writes a document; the GraphPlan gate launches its worksheet and writes a table. Both saved application files survive exported-image remount. The browser performs the same bounded launch sequences. Accountant and Nucleus are explicit multi-disc sets, while remaining candidate titles stay “media identified, not yet validated”.
